...
import {
  ...
  Observer,
  observer as observer$1,
} from './mobx-react-lite.js'
export {
  Observer,
  isObserverBatched,
  isUsingStaticRendering,
...
function createDefinition(
  ...
  return (
    (_ref = {}),
    (_ref[mobxPatchedDefinition] = true),
...
function makeClassComponentObserver(componentClass) {
  ...
  patch(target, 'componentWillUnmount', function () {
    ...
    if (this.render[mobxAdminProperty]) {
      ...
      console.warn(
        'The render function for an observer component (' +
          displayName +
          ") was modified after MobX attached. This is not supported, since the new function can't be triggered by MobX."
      )
    }
...
function inject() {
  for (
    var _len = arguments.length, storeNames = new Array(_len), _key = 0;
    _key < _len;
    _key++
  ) {
...
function runDisposersOnWillUnmount() {
  var _this = this
  ;[]
    .concat(this[protoStoreKey] || [], this[instStoreKey] || [])
    .forEach(function (propKeyOrFunction) {
      var prop =
        typeof propKeyOrFunction === 'string'
          ? _this[propKeyOrFunction]
          : propKeyOrFunction
...
function disposeOnUnmount(target, propertyKeyOrFunction) {
  ...
  if (
    !(
      c === React__default.Component ||
      c === React__default.PureComponent ||
      c2 === React__default.Component ||
      c2 === React__default.PureComponent
    )
  ) {
    throw new Error(
  ...
  var componentWasAlreadyModified =
    !!target[protoStoreKey] || !!target[instStoreKey]
